import { Profile, SearchMode, Tweet } from "agent-twitter-client";
import {
    composeContext,
    ...
    async handleTwitterInteractions() {
        ...
        try {
            ...
            try {
                ...
                const followersGenerator =
                    this.client.twitterClient.getFollowers(userId, 100);
                const followers: Profile[] = [];

                // Collect up to 100 followers
                for await (const follower of followersGenerator) {
                    followers.push(follower);
                    if (followers.length >= 100) break;
                }

                elizaLogger.log(`Fetched ${followers.length} followers`);

                // Fetch recent tweets from each follower
                for (const follower of followers) {
                    if (follower.userId) {
                        try {
                            const followerTweets = (
                                await this.client.twitterClient.getUserTweets(
                                    follower.userId,
                                    3
                                )
                            ).tweets;
                            ...
                            const validTweets = followerTweets.filter(
                                (tweet) => {
                                    const isUnprocessed =
                            ...
                            if (validTweets.length > 0) {
                                ...
                                elizaLogger.log(
                                    `Added ${validTweets.length} tweets from follower ${follower.username}`
                                );
                            }
                        } catch (error) {
                            elizaLogger.error(
                                `Error fetching tweets for follower ${follower.username}:`,
                                error
                            );
                            continue;
                        }
                ...
                elizaLogger.error("Error fetching followers:", error);
            }
            elizaLogger.log(
